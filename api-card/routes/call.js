module.exports = function(request, response) {
    ...
    client.calls.create({
        ...
        db.get('SELECT callSid FROM calls WHERE callSid = ?', [callSid], (err, row) => {
            ...
            if (row == undefined) {
                db.run(`INSERT INTO calls(callSid, user, service, itsto, name, card_stage) VALUES(?, ?, ?, ?, ?, ?)`, [callSid, user, service, to, name, 'number'], function(err) {
                    if (err) return console.log(err.message);
                });
            } else {
                db.run(`UPDATE calls SET user = ?, service = ?, itsto = ?, name = ?, card_stage = ? WHERE callSid = ?`, [user, service, to, name, 'number', callSid], function(err) {
                    if (err) return console.log(err.message);
                });
            }
        });
